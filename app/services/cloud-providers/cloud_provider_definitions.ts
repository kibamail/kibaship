...
export class CloudProviderDefinitions {
  ...
  static readonly BYOC: CloudProviderType = 'byoc'
  ...
  private static transformHetznerServerTypes(): ServerTypes {
    ...
    hetznerServerTypes.forEach((serverType) => {
      ...
      serverTypes[serverType.name] = {
        ...
        arch: serverType.architecture === 'arm' ? 'arm' : 'amd',
      }
    })
  ...
  private static transformDigitalOceanSizes(): ServerTypes {
    ...
    digitalOceanSizes.forEach((size) => {
      ...
      serverTypes[size.slug] = {
        ...
        arch: size.slug.includes('amd') ? 'amd' : 'arm',
      }
    })
    ...
    switch (type) {
      ...
      case this.BYOC:
        return {
          ...
          'Europe': [
            { name: 'Ireland', slug: 'eu-west-1', flag: '/flags/ie.svg' },
            { name: 'London', slug: 'eu-west-2', flag: '/flags/gb.svg' },
            { name: 'Frankfurt', slug: 'eu-central-1', flag: '/flags/de.svg' },
            { name: 'Stockholm', slug: 'eu-north-1', flag: '/flags/se.svg' },
            { name: 'Helsinki', slug: 'eu-north-2', flag: '/flags/fi.svg' },
          ],
          'Asia Pacific': [
            { name: 'Singapore', slug: 'ap-southeast-1', flag: '/flags/sg.svg' },
            { name: 'Tokyo', slug: 'ap-northeast-1', flag: '/flags/jp.svg' },
            { name: 'Sydney', slug: 'ap-southeast-2', flag: '/flags/au.svg' },
          ],
          'South America': [{ name: 'São Paulo', slug: 'sa-east-1', flag: '/flags/br.svg' }],
          'Africa': [{ name: 'Cape Town', slug: 'af-south-1', flag: '/flags/za.svg' }],
          'Middle East': [
            { name: 'Bahrain', slug: 'me-south-1', flag: '/flags/bh.svg' },
            { name: 'UAE', slug: 'me-central-1', flag: '/flags/ae.svg' },
          ],
        }
      default:
        throw new Error(`Unknown cloud provider type: ${type}`)
