...
export class TerraformService {
   ...
   */
  protected async init(templateName: TerraformTemplate) {
    let path = 'resources/views/clusters/terraform'

    if (
      templateName === TerraformTemplate.KUBERNETES_BOOT ||
      templateName === TerraformTemplate.KUBERNETES_CONFIG
    ) {
      path = 'resources/views/clusters/kubernetes'
    }

    const terraformViewsPath = app.makePath(path)
  ...
  async generate(cluster: Cluster, templateName: TerraformTemplate): Promise<TerraformFile> {
    ...
    if (templateName === TerraformTemplate.KUBERNETES_BYOC) {
  ...
  private buildTemplateContext(cluster: Cluster): TemplateContext {
    ...
    return {
      ...
      volumes: cluster.nodes
        ?.map(
          (
            node // Used by: volumes.tf.edge ONLY
          ) =>
            node.storages.map((storage) => ({
              ...
              size:
                node.type === 'master'
                  ? cluster.controlPlanesVolumeSize
                  : cluster.workersVolumeSize,
