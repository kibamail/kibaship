  ...
  public async callback(ctx: HttpContext) {
    ...
    if (error) {
      return ctx.response.redirect(
        `/w/${workspace.slug}/cloud/providers?error=Failed to authenticate with Digital Ocean. Please try again.`
      )
    }

    const cloudProvider = await CloudProvider.create({
      name: response?.data.info.team_name || 'Digital Ocean',
      type: 'digital_ocean',
      workspaceId: workspace.id,
      providerImageProvisioningCompletedAt: DateTime.now(),
      credentials: {
    ...
    return ctx.response.redirect(
      `/w/${workspace.slug}/cloud/providers?success=Successfully connected digital ocean provider: ${cloudProvider?.name}.`
    )
  }
}
